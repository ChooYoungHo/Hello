...
/* #define TEST_MODE   // 테스트 필요없으면 이것만 주석처리
   #include "헤더.h"
       int main()
       {
       #ifdef TEST_MODE
...
객체지향
   ...
   - 클래스 : 객체를 만들기 위한 설계도. 객체가 가져여할 상태(데이터)와 동작(기능)을 하나로 묶어 놓은 것.
   - 객체지향의 4대 특성
      - 추상화
	       - 불필요한 세부사항을 숨기고, 핵심적인 부분만 보여주는 것.
		   - 코드의 복잡성을 낮출 수 있다. (= 이해하기가 쉬워진다.)
	  - 캡슐화
	       - 접근 제한자를 통한 데어터의 접근 제어
		   - 몰라도 되는 것은 안보여야 한다. (데이터를 보호하고 코드의 안정성을 높이고 결합도도 떨어뜨린다.)
	  - 상속
	       - 부모 클래스(기본 클래스)의 데이터와 기능을 물려받은 자식 클래스(파생 클래스)를 만드는 것.
...
클래스
   ...
   - 구성 요소
	  ...
	  - 소멸자 : 구조체나 객체가 해제될 때 자동으로 호출되는 특별한 함수
		   ...
		   - 클래스 내부에서 동적 할당한 메모리를 해제하는데 주로 사용 (메모리 릭 방지)
      - 맴버 변수 : 객체의 상태나 데이터를 저장하는 변수. (속성, '필드'라고 함)
	  - 맴버 함수 : 객체가 할 수 있는 동작이나 기능을 정의한 함수. ('메서드'라고도 함)
*/

/*
접근 제한자(Access Modifier)
   - 이 객체의 내부를 누구까지 볼수 있을 것인가를 설정
